...
public class Mengtekaluo extends MoveStrategy {
    ...
    public Move getMove(Game game, byte dice) {
		...
		//获取当前的玩家颜色
		final Piece_Type player = game.getNowPlayer().getTurn();
		//获取所有可行步法
		Map<Move, ChessBoard> legalMoves = MoveSearch.getLegalMovesByDice(board, player, dice);
		final List<Move> keyList = new ArrayList<Move>(legalMoves.keySet());
		//定义最好步法以及大数据获胜轮数
		Move bestMove = null;
		...
		for (int i = 0;i<keyList.size();i++){
			final ChessBoard boardStep = legalMoves.get(keyList.get(i));
			final int j = i;
    		Runnable task = () -> {
    ...
    private int MonCa(ChessBoard board, Piece_Type player) {
    	  // 对弈轮数
			int game = 50000;
			int pieceWinNum = 0;
